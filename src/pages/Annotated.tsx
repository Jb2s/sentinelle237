...
const Annotated = () => (
  <PageShell
    ...
    title="Annotés"
    meta={<span>{notes.length} annotations · classées par date</span>}
  >
      ...
      {notes.map((n) => (
        ...
        >
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            ...
            <span className="font-semibold text-primary-deep">{n.source}</span>
            <span>·</span>
          ...
          <div className="flex gap-3 bg-secondary/60 border-l-4 border-primary rounded-r-lg p-3">
            ...
            <p className="text-sm text-primary-deep italic">{n.highlight}</p>
          </div>
        </div>
      ))}
    </div>
